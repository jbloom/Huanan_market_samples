...
%DIF DEL paper_original_submission.tex   Sun Jul  2 18:12:43 2023
%DIF ADD paper.tex                       Sun Jul  2 18:20:17 2023
...
%DIF 52-53c52-54
\DIFdel{However, the number of SARS-CoV-2 reads is not consistently correlated with reads mapping to non-human susceptible species.
...
%DIF -------
\DIFadd{
However, the SARS-CoV-2 content of the environmental samples is generally very low: only 21 of 176 samples contain more than 10 SARS-CoV-2 reads, despite most samples being sequenced to depths exceeding $10^8$  total reads. %DIF > 
...
%DIF > 
Only one of the 14 samples with at least a fifth of the chordate mitochondrial material from raccoon dogs contains any SARS-CoV-2 reads, and that sample only has 1 of $\sim$200,000,000 reads mapping to SARS-CoV-2.} %DIF > 
%DIF -------
...
In addition, Chinese scientists published papers reporting that some of the earliest identified human cases in December 2019 had no link to the Huanan Seafood Market~\DIFdelbegin %DIFDELCMD < \citep{huang2020clinical,chen2020epidemiological}%%%
\DIFdelend \DIFaddbegin \citep{huang2020clinical,chen2020epidemiological,zhang2020viral}\DIFaddend .
...
\DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{Some papers have argued that the fact that many of the known human cases from December 2019 are centered around market indicates it was the original source of the virus~}\citep{worobey2022huanan}\DIFadd{, and that an early two-mutation polytomy in the SARS-CoV-2 phylogenetic tree suggests multiple zoonotic jumps there~}\citep{pekar2022molecular}\DIFadd{.
Other papers have argued that reports of earlier cases unlinked to the market~}\citep{huang2020clinical,chen2020epidemiological,zhang2020viral,ma2020SCMP} \DIFadd{and uncertainties about the completeness of case data provided by the Chinese government~}\citep{cohen2022anywhere,hvistendahl2023} \DIFadd{mean that the first human infections could have occurred upstream of the December 2019 cases at the market~}\citep{pekar2021timing,pipes2021assessing,bloom2021recovery}\DIFadd{.
}

\DIFadd{Despite these disputes, it }\DIFaddend is universally agreed that there were human cases of SARS-CoV-2 at the Huanan Seafood Market by mid December of 2019~\citep{li2020early}\DIFdelbegin \DIFdel{.
It is also known }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend that some animals susceptible to SARS-CoV-2 were sold at the market~\citep{xiao2021animal}.
The \DIFaddbegin \DIFadd{key }\DIFaddend unanswered question is if any animals were infected, and if so whether they infected humans or were infected by them.
...
A significant caveat of the Chinese CDC study is that all the samples were collected on January-1-2020 or later, which is \DIFaddbegin \DIFadd{probably }\DIFaddend \emph{at least} a month after the first human infections in Wuhan~\DIFdelbegin %DIFDELCMD < \citep{odni2022,zhang2020viral,van2020emergence,pipes2021assessing}%%%
\DIFdelend \DIFaddbegin \citep{zhang2020viral,van2020emergence,he2020evaluating,pipes2021assessing,pekar2021timing,odni2022,pekar2022molecular}\DIFaddend .
...
The counts of reads mapping \DIFaddbegin \DIFadd{uniquely }\DIFaddend to each mitochondrial genome for each sequencing run and sample are provided in Tables~\ref{tab:mito_composition_by_run} and \ref{tab:mito_composition_by_sample}, respectively.
...
\DIFaddbeginFL \DIFaddFL{See Figure~\ref{fig:theil_sen_corr} for a version of this plot that uses correlations based on a Theil-Sen estimator that is more robust to outliers.
}\DIFaddendFL \label{fig:overall_corr}
}
\end{figure}
...
\DIFaddbegin \DIFadd{The trends are broadly similar if the correlations are instead calculated based on a Theil-Sen estimator~}\citep{theilsen} \DIFadd{that is more robust to outliers (Figure~\ref{fig:theil_sen_corr}).
}\DIFaddend 
...
It appears that the Chinese CDC classified samples as positive if they met \emph{either} of two criteria: they tested positive for SARS-CoV-2 by RT-qPCR, \emph{or} they were metagenomically sequenced and contained at least one read mapping to SARS-CoV-2 \DIFaddbegin \DIFadd{(Table~\ref{tab:rt-qpcr} gives sequencing and RT-qPCR data for all samples that were either sequenced or called positive by the Chinese CDC)}\DIFaddend .
...
\DIFdelbegin \DIFdel{See Table~\ref{tab:rt-qpcr} for merged sequencing and }\DIFdelend \DIFaddbegin 

\DIFadd{To see the lack of statistical rationale for classifying Q61 as positive but E-10-29-2 as negative (Table~\ref{tab:consistency}), we can use a Fisher exact test (or alternatively a $\chi^2$ test) to test the null hypothesis that the reads in samples Q61 and E-10-29-2 are drawn from the same underlying distribution of viral and non-viral reads given that we observe 1 in $2.1 \times 10^8$ SARS-CoV-2 reads in Q61 and 0 in $1.9 \times 10^8$ SARS-CoV-2 reads in E-10-29-2.
...
The lack of rationale is even more obvious when comparing Q61 and A1 (Table~\ref{tab:consistency}): both samples were negative by }\DIFaddend RT-qPCR\DIFdelbegin \DIFdel{data for all samples that were either sequencedor }\DIFdelend \DIFaddbegin \DIFadd{, but Q61 was called positive based on metagenomic sequencing whereas A1 was never sequenced.
Therefore, if Q61 is }\DIFaddend called as positive \DIFdelbegin \DIFdel{by the Chinese CDC}\DIFdelend \DIFaddbegin \DIFadd{then A1 should be called as indeterminate, since it was never subjected to the assay used to call Q61 as positive}\DIFaddend .
...
Mitochondrial material from most susceptible non-human species sold live at the market is negatively correlated with the presence of SARS-CoV-2: \DIFaddbegin \DIFadd{for instance, }\DIFaddend 13 of the 14 samples with at least \DIFdelbegin \DIFdel{20\% }\DIFdelend \DIFaddbegin \DIFadd{a fifth }\DIFaddend of their chordate mitochondrial material from raccoon dogs contain no SARS-CoV-2 reads, and the other sample contains just 1 of $\sim$200,000,000 reads mapping to SARS-CoV-2.
\DIFdelbegin \DIFdel{Likewise, none of the six samples with at least 20\% bamboo rat mitochondrial material contain any SARS-CoV-2 reads.
}\DIFdelend These findings are compatible with the results in \citet{crits2023genetic}, since that study did not report any analysis of SARS-CoV-2 content.
...
The first human infections with SARS-CoV-2 in Wuhan \DIFaddbegin \DIFadd{probably }\DIFaddend occurred no later than November of 2019~\DIFdelbegin %DIFDELCMD < \citep{odni2022,zhang2020viral,van2020emergence,pipes2021assessing}%%%
\DIFdelend \DIFaddbegin \citep{zhang2020viral,van2020emergence,he2020evaluating,pipes2021assessing,pekar2021timing,odni2022,pekar2022molecular}\DIFaddend , which is over a month before the Chinese CDC reports that it began to collect samples from the market.
...
\DIFadd{Note that a similar analysis could also be implemented using more standard metagenomic analysis tools such as Kraken2~}\citep{wood2019improved}\DIFadd{, as was in fact done in the Chinese CDC study~}\citep{liu2023surveillance}\DIFadd{.
The reasons for instead implementing the pipeline used here were two-fold.
First, to ensure comparability to} 
\citet{crits2023genetic} \DIFadd{
by using an approach that parallels the one they described.
...
The \DIFdelbegin \DIFdel{raccoon dog }\DIFdelend \DIFaddbegin \DIFadd{Amur hedgehog }\DIFaddend mitochondrial genome was not present in this set, so it was separately downloaded from Genbank accession KX964606.
...
\DIFaddbegin \DIFadd{Note that one effect of counting only primary alignments with mapping quality $>$0 (4 in this case) is to only count reads that map uniquely---that is, they map better to the mitochondrial genome of one species than all other species in the reference.
This is because although }\texttt{\DIFadd{minimap2}} \DIFadd{assigns the primary mapping randomly  if there are multiple sequences in the reference with equally good mappings, a read will only have a mapping quality that exceeds zero if it maps to one location better than all other locations.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{I thank Benjamin F. Pierce for identifying an error in the reference genome description in the original version (}\url{https://github.com/jbloom/Huanan_market_samples/pull/3}\DIFadd{).
}\DIFaddend This study utilized the Fred Hutch Scientific Computing infrastructure, which is supported in part by NIH grants S10-OD-020069 and S10-OD-028685.
The author is an Investigator of the Howard Hughes Medical Institute.

\section{Competing interests}
JDB is on the scientific advisory boards of Apriori Bio, Aerium Therapeutics, Invivyd, \DIFaddbegin \DIFadd{and }\DIFaddend the Vaccine Company\DIFdelbegin \DIFdel{, and Oncorus}\DIFdelend ; consults for GSK; and receives royalty payments as an inventor on Fred Hutch licensed patents related to deep mutational scanning of viral proteins.
...
\begin{figure*}
\DIFaddbeginFL \includegraphics[width=\linewidth]{figures/crits_christoph_vs_current_run_corr.pdf}
\caption{
\DIFaddFL{Correlation of the number of read counts mapping to mitochondrial genomes of mammalian species between the current study and}
\citet{crits2023genetic} 
\DIFaddFL{
for each sequencing run shared across the studies.
The correlation is calculated only across species for which}
\citet{crits2023genetic} \DIFaddFL{
provide read counts in the third supplementary figure of their report, and only for runs for which both studies find $\ge$50 reads mapping to that set of species.
See }\url{https://jbloom.github.io/Huanan_market_samples/crits_christoph_vs_current_run_corr.html} \DIFaddFL{for an interactive version of this plot that enables mouseover of points for details on specific runs, and adjustment of the threshold for how many total aligned mitochondrial reads a run must contain to be shown in this plot.
...
\caption{
\DIFaddFL{A version of Figure~\ref{fig:overall_corr} where the y-axis shows a correlation computed from the Theil-Sen estimator~}\citep{theilsen} \DIFaddFL{rather than the Pearson correlation.
...
See }\url{https://jbloom.github.io/Huanan_market_samples/theil_sen_corr.html} \DIFaddFL{for an interactive version of this plot.
